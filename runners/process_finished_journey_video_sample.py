...
async def execute(
    ...
    """Finalizes processing for a journey video sample where we've already produced the "raw"
    video sample and uploaded it to s3. This involves performing our standard video processing
    (which produces a variety of exports), uploading those to s3, and storing them in our
    database. When this completes successfully, we swap the journey's video sample, deleting
    ...
    with temp_file() as source_filepath:
        ...
        await slack.send_ops_message(
            f"Finalizing video sample for {journey_uid=} on {socket.gethostname()}"
        )
        ...
        try:
            result = await videos.process_video(
                ...
                name_hint="journey_video_sample",
        ...
        for _ in range(10):
            response = await cursor.execute(
                ...
                SELECT
                    content_files.uid
                FROM journeys
                LEFT OUTER JOIN content_files ON content_files.id = journeys.sample_content_file_id
                WHERE journeys.uid=?
                """,
                (journey_uid,),
            )
            ...
            if not response.results:
                await handle_warning(
                    ...
                    f"Journey {journey_uid=} was deleted while processing its video sample",
                )
                jobs = await itgs.jobs()
                await jobs.enqueue("runners.delete_content_file", uid=result.uid)
                return

            old_sample_content_file_uid: Optional[str] = response.results[0][0]

            response = await cursor.execute(
                """
                UPDATE journeys
                SET sample_content_file_id=content_files.id
                FROM content_files
                WHERE
                    journeys.uid = ?
                    AND content_files.uid = ?
                    AND (
                        (? IS NULL AND journeys.sample_content_file_id IS NULL) 
                        OR EXISTS (
                            SELECT 1 FROM content_files AS cf
                            WHERE cf.id = journeys.sample_content_file_id
                              AND cf.uid = ?
                        )
                    )
                """,
            ...
            await handle_warning(
                f"{__name__}:failed_to_swap_video_sample",
                f"Failed to swap video sample for {journey_uid=} after 10 attempts",
            )
            return

        if (
            old_sample_content_file_uid is not None
            and old_sample_content_file_uid != result.uid
        ):
            jobs = await itgs.jobs()
            await jobs.enqueue(
