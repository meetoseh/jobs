"""Success handler for onboarding video thumbnail images"""
...
from runners.generate_onboarding_video_thumbnails import TARGETS
...
async def execute(
    ...
    *,
    ...
    """Processes the s3 file upload with the given uid as a onboarding video thumbnail image.
    ...
    async def bounce():
        ...
        await jobs.enqueue(
            "runners.process_onboarding_video_thumbnail",
        ...
        with temp_file() as stitched_path:
            ...
            try:
                image = await process_image(
                    ...
                    name_hint=f"onboarding_video_thumbnail__user",
                    job_progress_uid=job_progress_uid,
                )
