...
a course video. Does not necessarily have to be a row in `course_videos`, though
...
SELECT
    courses.title,
    instructors.name
FROM content_files, courses, instructors
WHERE
    content_files.uid = ?
    AND courses.video_content_file_id = content_files.id
    AND NOT EXISTS (
        SELECT 1 FROM courses AS c2
        WHERE
            c2.video_content_file_id = courses.video_content_file_id
            AND (
                c2.created_at > courses.created_at
                OR (
                    c2.created_at = courses.created_at
                    AND c2.uid < courses.uid
                )
            )
    )
    AND instructors.id = courses.instructor_id
        ...
        course_response = response.items[3]
        ...
        if course_response.results:
            course_title = cast(str, course_response.results[0][0])
            instructor_name = cast(str, course_response.results[0][1])

        transcript_prompt: Optional[str] = None
        if course_title is not None and instructor_name is not None:
            transcript_prompt = f"Introduction to {course_title} with {instructor_name}"
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async with Itgs() as itgs:
            ...
            await jobs.enqueue(
                "runners.generate_course_transcript", content_file_uid=content_file_uid
            )
