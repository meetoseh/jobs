"""Deletes no longer needed journey monthly counts from redis to save memory"""
...
async def execute(itgs: Itgs, gd: GracefulDeath):
    ...
    earliest_stored = await redis.get("stats:journeys:monthly:earliest")
    ...
    for unix_month in range(earliest_unix_month, curr_unix_month):
        key = f"stats:journeys:monthly:{unix_month}:count"
        logging.debug(f"redis delete {key=}")
        await redis.delete(key)

    await redis.set("stats:journeys:monthly:earliest", curr_unix_month)
