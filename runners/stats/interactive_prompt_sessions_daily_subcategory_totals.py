...
async def execute(itgs: Itgs, gd: GracefulDeath):
    """Rotates the completed daily by-subcategory totals into the total by-subcategory totals,
    i.e., `stats:interactive_prompt_sessions:bysubcat:totals:{unix_date}` into
    `stats:interactive_prompt_sessions:bysubcat:totals`. The values in
    ``stats:interactive_prompt_sessions:bysubcat:totals` aren't incremented directly by the
    ...
    earliest_key = "stats:interactive_prompt_sessions:bysubcat:totals:earliest"
    earliest_raw = await redis.get(earliest_key)
    if earliest_raw is None:
        return

    earliest_unix_date = int(earliest_raw)
    overall_totals_key = "stats:interactive_prompt_sessions:bysubcat:totals"

    for unix_date in range(earliest_unix_date, curr_unix_date):
        if gd.received_term_signal:
            return

        totals_key = f"stats:interactive_prompt_sessions:bysubcat:totals:{unix_date}"
