"""Rotates client flow stats to the database"""
...
BREAKDOWN_EVENTS = ("triggered", "replaced")
...
def key_for_date(unix_date: int) -> bytes:
    return f"stats:client_flows:daily:{unix_date}".encode("ascii")


def key_for_date_and_event(unix_date: int, event: str) -> bytes:
    return f"stats:client_flows:daily:{unix_date}:extra:{event}".encode("ascii")
...
if __name__ == "__main__":
    ...
    async def main():
        async with Itgs() as itgs:
            ...
            await jobs.enqueue("runners.stats.daily_client_flow_stats")
