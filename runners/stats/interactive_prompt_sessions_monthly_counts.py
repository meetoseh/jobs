...
async def execute(itgs: Itgs, gd: GracefulDeath):
    """Deletes unused monthly interactive prompt session statistics from redis. This information
    ...
    earliest_key = "stats:interactive_prompt_sessions:monthly:earliest"
    ...
    async with redis.pipeline() as pipe:
        ...
        for month in range(earliest_stored_month, curr_unix_month, 1):
            key = f"stats:interactive_prompt_sessions:monthly:{month}:count"
