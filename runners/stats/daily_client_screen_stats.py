"""Rotates client screen stats to the database"""
...
BREAKDOWN_EVENTS = ("queued", "peeked", "popped", "traced")
"""Which events are broken down by an additional dimension."""


async def execute(itgs: Itgs, gd: GracefulDeath):
    """Rotates any overdue client screen statistics from redis to the database. Specifically,
...
def key_for_date(unix_date: int) -> bytes:
    return f"stats:client_screens:daily:{unix_date}".encode("ascii")


def key_for_date_and_event(unix_date: int, event: str) -> bytes:
    return f"stats:client_screens:daily:{unix_date}:extra:{event}".encode("ascii")
...
if __name__ == "__main__":
    ...
    async def main():
        async with Itgs() as itgs:
            ...
            await jobs.enqueue("runners.stats.daily_client_screen_stats")
