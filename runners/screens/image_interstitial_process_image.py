...
def _get_img_height(lw: int, lh: int, pr: Fraction) -> Fraction:
    return Fraction(_get_content_width(lw), 1) * Fraction(215, 342) * pr


RESOLUTIONS = list(
    dict.fromkeys(
        [
            (342, 215),  # preview, thumbhash
            *get_sizes_for_devices_newer_than(
                "2019-01-01",
                mapper=lambda lw, lh, pr: (
                    math.ceil(_get_content_width(lw) * pr),
...
async def execute(
    ...
    *,
    ...
    async def bounce():
        ...
        await jobs.enqueue(
            "runners.screens.image_interstitial_process_image",
    ...
    async with success_or_failure_reporter(itgs, job_progress_uid=job_progress_uid):
        with temp_file() as stitched_path:
            ...
            try:
                await process_interstitial_image(
                    itgs,
                    gd,
...
async def process_interstitial_image(
    itgs: Itgs,
    gd: GracefulDeath,
    *,
    stitched_path: str,
