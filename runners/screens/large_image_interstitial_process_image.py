...
def _get_content_width(lw: int) -> int:
    return min(lw - 24, 342)
        ...
        [
            ...
            *get_sizes_for_devices_newer_than(
                ...
                mapper=lambda lw, lh, pr: [
                    (math.ceil(w), math.ceil(h)) for w, h in _get_img_sizes(lw, lh, pr)
                ],
...
async def execute(
    ...
    *,
    ...
    """Processes the s3 file upload with the given uid for the image on an image
    interstitial screen.
    ...
    async def bounce():
        ...
        await jobs.enqueue(
            "runners.screens.large_image_interstitial_process_image",
    ...
    async with success_or_failure_reporter(itgs, job_progress_uid=job_progress_uid):
        with temp_file() as stitched_path:
            ...
            try:
                await process_large_interstitial_image(
                    itgs,
                    gd,
...
async def process_large_interstitial_image(
    itgs: Itgs,
    gd: GracefulDeath,
    *,
    stitched_path: str,
    ...
    """Processes the image at the given filepath to be used as an image in a
    large image interstitial screen. This will attempt to detect if alpha is needed
    ...
    list_slug = "large_image_interstitial"
