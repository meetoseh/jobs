"""Processes a raw video file intended to be used as an onboarding video"""
...
async def execute(
    ...
    *,
    ...
    """Processes the s3 file upload with the given uid as an onboarding video.
    ...
    async def bounce():
        ...
        await jobs.enqueue(
            "runners.process_onboarding_video_upload",
    ...
    async with success_or_failure_reporter(
        ...
        cv_uid = f"oseh_ovu_{secrets.token_urlsafe(16)}"
        last_uploaded_at = time.time()
        await cursor.execute(
            """
            INSERT INTO onboarding_video_uploads (
                uid,
                content_file_id,
                uploaded_by_user_id,
                last_uploaded_at
