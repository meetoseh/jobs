"""Checks the backend is still responding to /api/1 requests with 200 OK"""
...
async def execute(
    itgs: Itgs, gd: GracefulDeath, *, num_requests: int = 4, path: str = "/api/1"
):
    """Verifies the backend is still responding to /api/1 requests with 200 OK.
    ...
    """
    url = os.environ["ROOT_BACKEND_URL"] + path
    try:
    ...
    if num_succeeded != num_requests:
        warning = f"Backend health check *failed*: {num_succeeded}/{num_requests} succeeded:\n```{json.dumps(statuses)}```"
        if os.environ["ENVIRONMENT"] == "dev":
            logging.warning(warning)
        else:
            await handle_warning(f"{__name__}:failed", warning, is_urgent=True)
    else:
        logging.debug("Backend health check succeeded")
...
if __name__ == "__main__":
    ...
    async def main():
        async with Itgs() as itgs:
            ...
            await jobs.enqueue("runners.alerting.check_backend")
