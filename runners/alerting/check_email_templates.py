...
async def execute(
    ...
    *,
    num_requests: int = 4,
    path: str = "/api/3/management/hello_world",
):
    """Verifies the email-templates servers are still responding to
    /api/3/management/hello_world requests with 200 OK. Does 4 requests in
    ...
    if num_succeeded != num_requests:
        warning = f"email-templates health check *failed*: {num_succeeded}/{num_requests} succeeded:\n```{json.dumps(statuses)}```"
        if os.environ["ENVIRONMENT"] == "dev":
            logging.warning(warning)
        else:
            await handle_warning(f"{__name__}:failed", warning, is_urgent=True)
    else:
        logging.debug("email-templates health check succeeded")
...
if __name__ == "__main__":
    ...
    async def main():
        async with Itgs() as itgs:
            ...
            await jobs.enqueue("runners.alerting.check_email_templates")
