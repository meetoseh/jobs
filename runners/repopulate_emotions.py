...
import secrets
import string
from typing import AsyncIterator, FrozenSet, List, Literal, Optional, Tuple
...
the class resolves. You must output a numbered list containing at least 2
...
def parse_emotions(completion: str) -> FrozenSet[str]:
    ...
    for emotion in raw_emotions:
        emotion = emotion.lower().strip(string.whitespace + string.punctuation)
        if emotion not in EMOTIONS_SET:
            logging.debug(f"Dropping completed emotion {emotion=}: not a valid tag")
        else:
    ...
    *,
    ...
    while True:
        try:
            ...
            emotions = parse_emotions(completion.choices[0].message.content)
            logging.info(f"Parsed emotions: {json.dumps(emotions)}")
            return emotions
        except Exception:
            attempt += 1
...
async def assign_emotions_for_journey(
    ...
    *,
    ...
    await store_emotions_for_journey(
        itgs, uid=journey_uid, title=title, instructor=instructor, emotions=emotions
    )
