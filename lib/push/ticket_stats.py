...
PUSH_TICKET_STATS_EVENTS = frozenset(
    (
        "queued",
        "retried",
        "succeeded",
...
async def increment_event(
    itgs: Itgs, *, event: PushTicketStatsEvent, now: float, amount: int = 1
) -> None:
...
async def attempt_increment_event(
    ...
    *,
    ...
    Args:
        ...
        amount (int, optional): The amount to increment by. Defaults to 1.
    """
    if event not in PUSH_TICKET_STATS_EVENTS:
        raise ValueError(f"Invalid event: {event}")

    unix_date = unix_dates.unix_timestamp_to_unix_date(now, tz=timezone)
    await set_if_lower(client, b"stats:push_tickets:daily:earliest", unix_date)
