"""Assists with updating statistics related to managing expo push tokens and
...
# Documentation for these events is under `docs/db/stats/push_token_stats.md`
PushTokenStatsEvent = Literal[
    "created",
    "reassigned",
    "refreshed",
...
async def prepare_increment_event(client: redis.asyncio.Redis, *, force: bool = False):
    ...
    increment a push token stats event. This has to be done outside of
...
async def attempt_increment_event(
    ...
    *,
    ...
    Args:
        ...
        amount (int, optional): The amount to increment by. Defaults to 1.
    """
    if event not in PUSH_TOKEN_STATS_EVENTS:
